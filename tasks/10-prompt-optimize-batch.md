...
- Implement a new command-line option for batch prompt optimization:
  - Input: CSV file with a column containing prompts to optimize
  - Output: Directory for storing optimized prompts

- For each input prompt:
  1. Generate an optimized version
  2. Infer the prompt's category and name
  3. Create a YAML file with the optimized prompt:
     - Structure: Subfolder named after inferred category
     - Filename: Inferred prompt name (with .yaml extension)
     - Content: Optimized prompt in YAML format

- Filename collision handling:
  - If a file with the same name exists, append an incremental number
  - Example: prompt.yaml, prompt_1.yaml, prompt_2.yaml, etc.
